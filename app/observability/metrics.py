...
Metrics are written to:
  ...
  3. A separate HIPAA audit log file (configurable via env HIPAA_AUDIT_LOG)
...
Call metrics.audit(user_id, action, resource, **kwargs) for HIPAA audit events.
"""
...
_AUDIT_LOG_FILE: str = os.getenv("HIPAA_AUDIT_LOG", "hipaa_audit.jsonl")
_BUFFER_SIZE: int = 10_000
_ENABLED: bool = os.getenv("LTM_METRICS_ENABLED", "true").lower() == "true"


class _Metrics:
    """Thread-safe, zero-dependency metrics collector with HIPAA audit log."""
    ...
    def __init__(self) -> None:
        ...
        # Separate HIPAA audit log file handle
        if _AUDIT_LOG_FILE:
            try:
                self._audit_handle = open(_AUDIT_LOG_FILE, "a", buffering=1)
            except OSError:
                pass
    ...
    def audit(
        ...
        **kwargs: Any,
        ...
        """
        Record a HIPAA audit event.
        Appends to the separate audit log file and increments metrics counter.
        """
        record = {
            "ts": datetime.utcnow().isoformat() + "Z",
            "event": "hipaa_audit",
        ...
        with self._lock:
            self._events.append(record)
            self._counts["hipaa_audit"] += 1
            if self._audit_handle:
                try:
                    self._audit_handle.write(json.dumps(record) + "\n")
                except OSError:
                    pass
